...
async def post_application(application: ApplicationCreate, db: AsyncSession = Depends(get_db)):
...
async def get_all_applications(page: int = 1, size: int = 10, db: AsyncSession = Depends(get_db)):
    return await get_all(db, page, size)


@router.get("/{name}", response_model=List[ApplicationResponse], summary="Поиска заявки по имени")
async def get_application_by_name(user_name: str, page: int = 1, size: int = 10,  db: AsyncSession = Depends(get_db)):
    return await get_application(db, user_name, page, size)


@router.delete("/{name}", summary= "Удаление заявки по имени")
async def delete_application_by_name( user_name: str, db: AsyncSession = Depends(get_db)):
